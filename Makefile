...
install-app:
	...
	printf "%s\n" "#!/bin/sh" "cd \"$(APP_DIR)\" || exit 1" "exec python3 gui.py \"$$@\"" > "$(BIN_FILE)"
	chmod +x "$(BIN_FILE)"
...
run:
	@if [ -x "$(BIN_FILE)" ]; then setsid "$(BIN_FILE)" >/tmp/RSLManagerForLinux.log 2>&1 & else setsid python3 gui.py >/tmp/RSLManagerForLinux.log 2>&1 & fi
